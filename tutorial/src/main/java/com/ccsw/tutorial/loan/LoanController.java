...
public class LoanController {
    // Inyecta loanService y mapper
    @Autowired
    LoanService loanService;
    // Para transformar en Dto
    @Autowired
    ModelMapper mapper;

    @Operation(summary = "Find Page", description = "Method that return a page of Loans")
    @RequestMapping(path = "", method = RequestMethod.POST)
    // LoanSearchDto almacena toda la información necesaria para la búsqueda
    public Page<LoanDto> findPage(@RequestBody LoanSearchDto dto) {

        Page<Loan> page = this.loanService.findPage(dto);
        // Se abre un stream para pasar los datos, se transforman en dto con map y filtran
        // Finalmente con collect se agrupan en una lista y cierra el stream
